...
export default function MetricDetailScreen() {
  ...
  const yAt = (v: number) => {
    const c = Math.max(series.yMin, Math.min(series.yMax, Number.isFinite(v) ? v : series.yMin));
    return PAD_T + (1 - (c - series.yMin) / (series.yMax - series.yMin || 1)) * plotH;
  };

  // 折线分段（遇 null 断线）；同时把所有真实点收集为散点，保证稀疏/单点历史日也能看见。
  const { lineD, areaD, last, dots } = useMemo(() => {
    if (w === 0 || n === 0) return { lineD: '', areaD: '', last: { x: 0, y: 0 }, dots: [] };
    if (n === 1) {
  ...
  }, [w, n, series.points, series.yMin, series.yMax]);
          ...
          <View onLayout={(e: LayoutChangeEvent) => setW(e.nativeEvent.layout.width)}>
            {w > 0 ? (
              <Svg width={w} height={H} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
                ...
                {/* 量程参考线（yMin / 中 / yMax） */}
                {[series.yMin, (series.yMin + series.yMax) / 2, series.yMax].map((gv, i) => (
                  <Line
                    key={`g-${i}`}
                    x1={PAD_L}
                    y1={yAt(gv)}
...
const styles = StyleSheet.create({
  ...
  axis: { flexDirection: 'row', justifyContent: 'space-between', marginTop: theme.sp(2) },
